...
for (let i = 0; i < choose.length; i++) {
    ...
    createBoxes(): void {
        ...
        this.box2 = this.physics.add.image(1025, 400, MedfruitList[this.gameNumber][1].name);
        this.box3 = this.physics.add.image(1025, 550, MedfruitList[this.gameNumber][2].name);
    ...
    restoreBoxPosition(): void {
        ...
        this.box3?.setPosition(1030,550)
    ...
    goResult(): void {
        ...
        const storedScore = Number(localStorage.getItem('medhighestScore'));
        ...
        if (isNaN(storedScore) || storedScore < score) {
            localStorage.setItem('medhighestScore', score.toString());
        }
    
        // Stop the scene and navigate to the result page with the score in the URL
        this.scene.stop();
        window.location.href = `/ShootingGame/Result?score=${score}&difficulty=medium`;
    }
